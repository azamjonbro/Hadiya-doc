 ...
 * `cmi.success_status`. Scores move too, and the two time formats are not
 * even the same shape. Translating once — on the way in — means reports, the
 * curriculum row and the completion rule all read the same three fields,
 * whatever the package was exported from.
 *
 * Pure, and in its own file, for the same reason lessonBlocks.js is: the
 * completion service needs `meetsPackage()` and the runtime service needs
 * the completion service, so a shared module is what keeps those two from
