 ...
 * Our own Tooltip rather than `title`: the browser's waits a second and
 * looks like a system hint, and a rail of unlabeled icons needs the name
...
<template>
  <nav class="z-20 flex h-full w-14 shrink-0 flex-col items-center border-r border-border bg-surface pt-3" :aria-label="t('a11y.mainNav')">
    <ul class="flex flex-1 flex-col items-center gap-1">
      <li v-for="section in sections" :key="section.key" class="relative">
        ...
        <Tooltip :text="t(section.labelKey)" position="right">
          <router-link
            :to="section.path"
    ...
    <div class="flex flex-col items-center gap-2 pb-3">
      ...
      <Tooltip :text="t('settings.adminPanel.backToUser')" position="right">
        <router-link
          to="/"
      ...
      <Tooltip :text="auth.user?.fullName ?? ''" position="right">
        <router-link to="/profile" class="rounded-md" :aria-label="auth.user?.fullName ?? ''">
