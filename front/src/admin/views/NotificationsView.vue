  ...
  <div class="mx-auto max-w-3xl px-6 py-8">
    ...
    <div class="mt-5 divide-y divide-border rounded-lg border border-border bg-surface">
      ...
      <template v-else>
        ...
        <EmptyState v-if="filtered.length === 0" icon="bell" :title="t('admin.notifications.empty')" />
      </template>
    </div>

    <div v-if="nextCursor" class="mt-4 flex justify-center">
      <AppButton variant="outline" size="sm" @click="loadMore">{{ t('users.loadMore') }}</AppButton>
    </div>
  </div>
</template>
