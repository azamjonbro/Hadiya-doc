...
export const tasksApi = {
  ...
  // Every copy of every task the caller assigned, unpaginated — the board
  // groups fan-outs into one card per batch itself, and needs the
  // individual rows to show who inside a group is done.
  listBoard() {
    return http.get('/tasks/board').then((r) => r.data.data)
  },
  // `fromStatus` is the column the card was dragged out of, so only the
  // copies that were actually in it move.
  ...
  // The kanban feed: every task this admin assigned, uncapped by a cursor so
  // the columns and the per-card recipient tallies are complete.
  listBoard() {
    return http.get('/tasks/assigned-by-me/board').then((r) => r.data.data)
  },
  // Moving or deleting a fan-out card reaches every copy in one request.
  // `fromStatus` is the column the card was dragged out of.
  updateBatch(batchId, payload) {
    return http.patch(`/tasks/batch/${batchId}`, payload).then((r) => r.data.data)
  },
  removeBatch(batchId, params) {
