...
| Malicious file upload | MIME/magic-byte sniffing (`file-type`) beyond the extension, file-size caps, storage keys are server-generated UUIDs (never derived from the uploaded filename), processing happens in the isolated worker process, originals are never publicly served |
| NoSQL injection / XSS | Mongoose's parameterized query builders (no raw string interpolation into queries), Zod validation on every request body/query/param, DOMPurify sanitization on any stored HTML (news body) before persistence, Vue's default template auto-escaping on render |
...
| Path traversal | Object storage keys are always server-generated UUIDs/derived paths — user-supplied filenames are stored as metadata only, never used to build a filesystem or bucket path |
...
| AI endpoint context leakage / abuse | The `/ai/chat` route runs through the exact same authorization middleware as content routes — the AI is only ever given context for a `courseId`/`topicId`/`videoId` the caller actually has an active assignment for; a strict per-user rate limit applies given per-call cost |
